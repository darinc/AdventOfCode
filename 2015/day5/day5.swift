// Advent of code Day5 Swift Playground
...
var p1_nice_strings = 0
var p2_nice_strings = 0
var p2_passing_strings = [String]()

for word in strings {

    var p1_vowels = 0
    var p1_badstring = false
    var p1_doubles = false
    var p2_repeat = false


    if word.rangeOfString("ab") != nil {
        p1_badstring = true
    } else if word.rangeOfString("cd") != nil {
        p1_badstring = true
    } else if word.rangeOfString("pq") != nil {
        p1_badstring = true
    } else if word.rangeOfString("xy") != nil {
        p1_badstring = true
    }
    ...
    print("\(p1_vowels) and \(p1_doubles) and \(p1_badstring)")
    if p1_vowels >= 3 && p1_doubles && !p1_badstring {
        p1_nice_strings++
    }
    if p2_repeat {
        p2_passing_strings.append(word)
    }
}

print("Part one: There are \(p1_nice_strings) nice strings")


var very_nice_strings = 0
for w in p2_passing_strings {
    var p2_doubles = 0
    ...
    for i in 0..<s.count {
        ...
        if i < s.count-1 {
            ...
            let range: Range<String.Index> = w.rangeOfString("\(s[i])\(s[i+1])", options: .BackwardsSearch)!
            let loc: Int = w.startIndex.distanceTo(range.startIndex)
...
print("Part two: There are \(very_nice_strings) very nice strings")
